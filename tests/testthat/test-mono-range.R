test_that("validate_mono_range works with valid input", {
  expect_no_error(validate_mono_range(list(Hex = c(3L, 9L)), mono_type = "generic"))
  expect_no_error(validate_mono_range(list(Hex = c(0L, Inf)), mono_type = "generic"))
  expect_no_error(validate_mono_range(NULL, mono_type = "generic"))
})

test_that("validate_mono_range errors on duplicate names", {
  expect_error(validate_mono_range(list(Hex = c(3L, 9L), Hex = c(2L, 6L)), mono_type = "generic"), "duplicated")
})

test_that("validate_mono_range errors on invalid min/max", {
  expect_error(validate_mono_range(list(Hex = c(-1L, 5L)), mono_type = "generic"), "min")
  expect_error(validate_mono_range(list(Hex = c(3L, 2L)), mono_type = "generic"), "max.*min")
  expect_error(validate_mono_range(list(Hex = c(Inf, 5L)), mono_type = "generic"), "min.*Inf")
})

test_that("validate_mono_range errors on non-integer values", {
  expect_error(validate_mono_range(list(Hex = c(3, 9)), mono_type = "generic"), "integer")
})

test_that("validate_mono_range errors on invalid monosaccharide names", {
  expect_error(validate_mono_range(list(InvalidMono = c(3L, 9L)), mono_type = "generic"), "known")
})
...
test_that("filter_by_mono_range works with compositions", {
  ...
  result <- filter_by_mono_range(comps, list(Hex = c(2L, 4L)), "generic")
...
test_that("filter_by_mono_range defaults missing monos to 0 for generic glycan compositions", {
  ...
  result <- filter_by_mono_range(comps, list(dHex = c(0L, 0L), Hex = c(0L, Inf)), "generic")
...
test_that("filter_by_mono_range defaults missing monos to 0 for concrete glycan compositions", {
  ...
  result <- filter_by_mono_range(comps, list(Glc = c(0L, 2L)), "concrete")
